...
window.addEventListener('DOMContentLoaded', (event) =>{
    ...
    let rander = document.getElementById("random");
        ...
        rander.onclick = Randomizeit
        function Randomizeit(){
            punch.real = (Math.random()-.5)*2
            punch.imaginary = (Math.random()-.5)*2
        }
        tutorial_canvas.style.background = "black"
       function pausfunc(){
            paused*=-1
            if(paused == 1){
                pauser.innerText = 'unpause'
            }else{
    ...
    let punch = new Complex(-.71,.2175)
    
    punch.real += .3*(Math.random()-.5)
    punch.imaginary += .3*(Math.random()-.5)
    ...
    for(let t = 0; t<boxes.length; t++){
        for(let k = 0; k<boxes[t].length;k++){
            ...
            while( iet<100 && ((boxes[t][k].complex.real*boxes[t][k].complex.real)-(boxes[t][k].complex.imaginary*boxes[t][k].complex.imaginary))<r*r){
            iet++
            let xtemp = (boxes[t][k].complex.real  * boxes[t][k].complex.real)  - (boxes[t][k].complex.imaginary  * boxes[t][k].complex.imaginary) 
            boxes[t][k].complex.imaginary = (2 * boxes[t][k].complex.real  * boxes[t][k].complex.imaginary)   + punch.imaginary 
            boxes[t][k].complex.real = xtemp + punch.real
            ...
            boxes[t][k].color = `rgb(${Math.min((iet*15),255)},${Math.max(255-(iet*3),0)}, ${Math.min((iet*(iet/50)),128)}, ${a}`
    ...
    for(let t = 0; t<boxes.length; t++){
        for(let k = 0; k<boxes[t].length;k++){
            ...
            if(a < .5){
                ...
                a=1 // turn off for transparencies, right click the canvas to save
            boxes[t][k].color = `rgb(${Math.min((iet*18),255)},${Math.max(255-(iet*3),0)}, ${Math.min((iet/(start)),255)}, ${a}`
    ...
    punch.real-=(.33 *(Math.random()-.5))// real component
    punch.imaginary+=(.33 *(Math.random()-.5))// imaginary component
            // tutorial_canvas_context.translate(350,350)
            // tutorial_canvas_context.rotate(.001)
            // tutorial_canvas_context.translate(-350,-350)
            // tutorial_canvas_context.scale(1.01,1.01)
...
let sipd = document.getElementById("disp");
window.setInterval(function(){ 
    let img = Math.round((punch.imaginary*10000))/10000
    let rel = Math.round((punch.real*10000))/10000
    sipd.innerText = `Real ${rel}, Imaginary ${img}`
 },  1) 
    })